...
//            sendSuccessResponse(response, successResponse);
            ...
            long totalPriceLong = totalPrice.longValue() * 100;
            ...
            Map<String, Object> successResponse = new HashMap<>();
            successResponse.put("redirectURL", paymentUrl);
            sendSuccessResponse(response, successResponse);
        }catch (Exception e){
            e.printStackTrace(); // In lỗi chi tiết
            response.setContentType("text/plain");
            response.setCharacterEncoding("UTF-8");
            response.getWriter().write("Lỗi khi xử lý thanh toán: " + e.toString()); // Thay vì e.getMessage()
        }
    }
